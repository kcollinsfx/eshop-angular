...
const mock_items = {
  items: [
    ...
    {id: 5, name: 'Under Armor', price: 30.0, category: 'Shoes', description: ''},
...
export class ItemService {
  ...
  getItems(page: number, pageSize: number, filter: Filter): Observable<ItemPayload> {
    let filteredItems: Item[] = mock_items.items.filter(item => {
      return (item.name.indexOf(filter.name) >= 0 && (filter.categories.length == 0 || filter.categories.includes(item.category)));
    });
